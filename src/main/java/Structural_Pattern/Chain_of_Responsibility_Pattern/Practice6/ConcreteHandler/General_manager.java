...
public class General_manager extends Spice_approval {
    public General_manager(String position) {
        super(position);
    }
    ...
    public void processResponse(Splendid splendid) {
        if (splendid.getDay()>=10&&splendid.getDay()<=30){
            System.out.println("处理者："+this.position);
        }else{
            System.out.println(this.position+"拒绝了");
        }
    }
}